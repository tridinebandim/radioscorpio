...
*/
$path = drupal_get_path('theme', 'outreach');
//require ($path . '/nowplaying/icecast.php');
require($path . '/nowplaying/config.php');
...
if($stream['info']['status'] == 'OFF AIR'){
	...
	if($last_song != base64_encode($stream['info']['song'])){
		...
		$stream = array_decode(json_decode(@file_get_contents($path . '/nowplaying/var/info.json'), TRUE));
	}
}
//print_r($stream);
...
function cacheVar($stream){
	...
	file_put_contents($path . '/nowplaying/var/info.json', json_encode($stream));
}
...
function createHistory(){
	$history = json_decode(@file_get_contents($path . '/nowplaying/var/history.json'), TRUE);
	$year = date('Y');
	$month = date('m');
	$day = date('d');
